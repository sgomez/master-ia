...
class MuyMalo(Vino):
    def __init__(self):
        ...
        self.add_rule(rules.ReglaRango('r5', self.get_attribute('Cloruros'), [1.2, 5.7]))
...
#        self.add_rule(rules.ReglaRango('r1', self.get_attribute('Acided fija'), [6, 12]))
#        self.add_rule(rules.ReglaRango('r2', self.get_attribute('Acided volatil'), [0, 2]))
#        self.add_rule(rules.ReglaRango('r3', self.get_attribute('Acido citrico'), [0, 3]))
#        self.add_rule(rules.ReglaRango('r4', self.get_attribute('Azucar residual'), [1, 6]))
#        self.add_rule(rules.ReglaRango('r5', self.get_attribute('Cloruros'), [1, 6]))
#        self.add_rule(rules.ReglaRango('r6', self.get_attribute('Dioxido de azufre libre'), [3, 34]))
#        self.add_rule(rules.ReglaRango('r7', self.get_attribute('Dioxido de azufre total'), [9, 49]))
#        self.add_rule(rules.ReglaRango('r8', self.get_attribute('Densidad'), [1, 4]))
#        self.add_rule(rules.ReglaRango('r9', self.get_attribute('pH'), [3, 5]))
#        self.add_rule(rules.ReglaRango('r10', self.get_attribute('Sulfatos'), [0, 4]))
#        self.add_rule(rules.ReglaRango('r11', self.get_attribute('Alcohol'), [8, 11]))